...
import time
import re
import pandas as pd # นำเข้าไลบรารีสำหรับอ่านไฟล์ Excel
...
# --- ฟังก์ชันบังคับใช้ Glossary (รับค่าเป็น Dictionary) ---
...
with st.sidebar:
    ...
    # --- อัปเกรด: เพิ่มที่อัปโหลดไฟล์ Excel ---
    ...
    # --- รวมคำศัพท์จากทั้ง Excel และที่พิมพ์สดเข้าด้วยกัน ---
    master_glossary = {}
    
    # 1. ดูดคำศัพท์จาก Excel (ถ้ามีคนอัปโหลด)
    if uploaded_file is not None:
        try:
            # อ่าน Excel โดยถือว่าคอลัมน์ A คือ Source และคอลัมน์ B คือ Target
            ...
            st.success(f"โหลดศัพท์สำเร็จ {len(df)} คำ!")
        except Exception as e:
            st.error("รูปแบบไฟล์ไม่ถูกต้อง โปรดใช้ Column 1 เป็นต้นฉบับ Column 2 เป็นคำแปล")

    # 2. ดูดคำศัพท์จากช่องพิมพ์สด (ถ้ามี)
    if glossary_input:
    ...
    st.divider()
    st.info("💡 ทริค: รองรับทั้งไฟล์ Excel และการพิมพ์สดพร้อมกัน")
...
if st.button("🚀 เริ่มการเกลาซับไตเติล", use_container_width=True):
    with st.spinner("🧠 AI กำลังคำนวณบริบทและตรวจสอบกฎ..."):
        ...
        # --- เอาผลลัพธ์มาผ่านเครื่องกรอง Glossary (ใช้ข้อมูลที่รวมมาแล้ว) ---
