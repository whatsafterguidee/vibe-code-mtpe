...
# --- ฟังก์ชันบังคับใช้ Glossary (อัปเกรดให้ฉลาดขึ้น ไม่สนตัวพิมพ์เล็ก/ใหญ่) ---
def apply_glossary(text, glossary_dict):
    ...
    for source_word, target_word in glossary_dict.items():
        if source_word.lower() == 'i':
            ...
            # ใช้ Regex เพื่อให้ค้นหาคำศัพท์แบบไม่สนตัวพิมพ์เล็ก-พิมพ์ใหญ่ (Case-Insensitive)
...
with st.sidebar:
    ...
    # --- อัปเกรด: อ่านไฟล์ Excel โดยบังคับให้มองแถวแรกเป็นข้อมูล (header=None) ---
    if uploaded_file is not None:
        try:
            # เพิ่ม header=None ระบบจะได้ไม่ตัดแถวที่ 1 ทิ้ง
            ...
            # โชว์ให้เห็นเลยว่าระบบดูดคำว่าอะไรมาบ้าง (เอาไว้โชว์ความโปรตอนพรีเซนต์)
            with st.expander("ดูรายการคำศัพท์ที่โหลดมา"):
                st.write(master_glossary)
                
        except Exception as e:
            st.error("เกิดข้อผิดพลาดในการอ่านไฟล์ โปรดลองใหม่อีกครั้ง")
